...
func TestNewExpiringWithTimeout(t *testing.T) {
	NewExpiringWithTimeout(42, 99*time.Second)
	//st.Expect(t, r.cache.capacity, 42)
	//st.Expect(t, r.timeout, 99*time.Second)
	//st.Expect(t, r.expire, true)
}
...
func TestResolverCache(t *testing.T) {
	...
	st.Expect(t, err, NXDOMAIN)
	st.Expect(t, rrs.AnswerRRs, ([]RR)(nil))
	//r.cache.m.Lock()
...
func TestGoogleA(t *testing.T) {
	...
	st.Expect(t, err, nil)
	st.Expect(t, len(rrs.AnswerRRs) >= 4, true)
	st.Expect(t, count(rrs.AnswerRRs, func(rr RR) bool { return rr.Type == "NS" }) >= 2, true)
	st.Expect(t, count(rrs.AnswerRRs, func(rr RR) bool { return rr.Type == "A" }) >= 1, true)
}
...
func TestGooglePTR(t *testing.T) {
	...
	st.Expect(t, err, nil)
	st.Expect(t, len(rrs.AnswerRRs) >= 2, true)
	st.Expect(t, count(rrs.AnswerRRs, func(rr RR) bool { return rr.Type == "PTR" }) >= 1, true)
}
...
func TestGooglePTRTwoLabelsDownDelegated(t *testing.T) {
	...
	st.Expect(t, err, nil)
	st.Expect(t, len(rrs.AnswerRRs) == 1, true)
	st.Expect(t, count(rrs.AnswerRRs, func(rr RR) bool { return rr.Type == "PTR" }) == 1, true)
	st.Expect(t, count(rrs.AnswerRRs, func(rr RR) bool { return rr.Value == "dns.google." }) == 1, true)
}
...
func TestGoogleMX(t *testing.T) {
	...
	st.Expect(t, len(rrs.AnswerRRs) >= 4, true)
	st.Expect(t, count(rrs.AnswerRRs, func(rr RR) bool { return rr.Type == "NS" }) >= 2, true)
	st.Expect(t, count(rrs.AnswerRRs, func(rr RR) bool { return rr.Type == "MX" }) >= 1, true)
}
...
func TestGoogleMulti(t *testing.T) {
	...
	st.Expect(t, count(rrs.AnswerRRs, func(rr RR) bool { return rr.Type == "TXT" }) >= 1, true)
	st.Expect(t, count(rrs.AnswerRRs, func(rr RR) bool { return rr.Type == "A" }), 0)
}
...
func TestGoogleCNAME(t *testing.T) {
	...
	st.Expect(t, err, nil)
	st.Expect(t, len(rrs.AnswerRRs) >= 1, true)
	st.Expect(t, count(rrs.AnswerRRs, func(rr RR) bool { return rr.Type == "CNAME" }), 1)     // resolved first
	st.Expect(t, count(rrs.AnswerRRs, func(rr RR) bool { return rr.Type == "A" }) >= 1, true) // records for CNAME target
}
...
func TestGoogleTXTTCPRetry(t *testing.T) {
	...
	st.Expect(t, err, nil)
	st.Expect(t, len(rrs2.AnswerRRs) > len(rrs.AnswerRRs), true)
}
...
func TestHerokuTXT(t *testing.T) {
	...
	st.Expect(t, err, nil)
	st.Expect(t, count(rrs.AnswerRRs, func(rr RR) bool { return rr.Type == "TXT" }), 0)
}
...
func TestBlueOvenA(t *testing.T) {
	...
	st.Expect(t, err, nil)
	st.Expect(t, len(rrs.AnswerRRs), 2)
	st.Expect(t, count(rrs.AnswerRRs, func(rr RR) bool { return rr.Type == "NS" && rr.Name == "blueoven.com." }), 2)
}
...
func TestBlueOvenMulti(t *testing.T) {
	...
	st.Expect(t, err, nil)
	st.Expect(t, len(rrs.AnswerRRs), 2)
	st.Expect(t, all(rrs.AnswerRRs, func(rr RR) bool { return rr.Type == "NS" }), true)
}
...
func TestTTL(t *testing.T) {
	...
	rrs, err := r.ResolveErr("google.com", "A")
	st.Expect(t, err, nil)
	st.Assert(t, len(rrs.AnswerRRs) >= 4, true)
	rr := rrs.AnswerRRs[0]
...
func checkTXT(t *testing.T, domain string) {
	...
	c := count(rrs.AnswerRRs, func(rr RR) bool { return rr.Type == "TXT" })
	if c != len(rrs2) {
